...
- Share Favorite Scripts button (only in Fav tab)
- Share Recent Scripts button (only in Rct tab)
...
- First release]],
    ...
    RU = {
        ...
        RctShared = "Недавние скрипты скопированы!",
...
- Share Favorite Scripts (только во вкладке Fav)
- Share Recent Scripts (только во вкладке Rct)
...
-- Share Favorite Scripts (только в Fav)
...
-- Share Recent Scripts (только в Rct)
...
function RefreshButtons()
    ...
    local showFavShare = (currentTab == "Fav")
    local showRctShare = (currentTab == "Rct")

    removeTagsContainer.Visible = showSpecial
    shareFavContainer.Visible = showFavShare
    shareRctContainer.Visible = showRctShare
    ...
    if showFavShare then
        shareFavContainer.Position = UDim2.new(0, 5, 0, visible * buttonHeight + 5)
        visible = visible + 1
    end
    if showRctShare then
        shareRctContainer.Position = UDim2.new(0, 5, 0, visible * buttonHeight + 5)
        visible = visible + 1
    end
...
local function ApplyTheme(themeName)
    ...
    for _, btn in ipairs({closeButton, langButton, playerBtn, serverBtn, customBtn, changelogButton, themeBtn, colorBtn}) do
        btn.BackgroundColor3 = RED_DARK
        btn.TextColor3 = RED_MAIN
        btn.BorderColor3 = RED_MAIN
    end
...
local function ShowWelcomeMessage()
    ...
    doc.Text = "─── TITLE BAR ───\n" ..
        ...
        "\n─── FAV TAB ───\n" ..
        "Share Favorite Scripts — copy favorites list\n" ..
        "\n─── RCT TAB ───\n" ..
        "Share Recent Scripts — copy recent list\n" ..
        "\n─── CUSTOM COLOR WINDOW ───\n" ..
        "Resize handle in bottom-right corner\n" ..
        "RGB sliders, HEX input, presets\n" ..
        "Reset, Share Color, Apply\n" ..
        "\n─── TOP BAR ───\n" ..
